...
const CardInterest = () => {
  return (
    <div className="dark:bg-base-main text-gray-900 shadow-md dark:text-white group rounded-md overflow-hidden w-full min-h-82 h-auto flex flex-col">
      <div className="min-h-40 md:min-h-1/2 h-1/2 w-full relative bg-transparent">
        <HoverSeeTrailer />
        <Image
          layout="fill"
          loader={({ src }) => `${src}`}
      ...
      <div className="flex-grow min-h-1/2 max-h-1/2 h-1/2 w-full box-border p-4 flex flex-col justify-between text-sm transition-all duration-200 ease-in dark:hover:bg-base-lighter-dark-mode dark:group-hover:bg-base-lighter-dark-mode">
